...
Each frame is RGB24 and ready for your own rendering, encoding, or processing.
...
- Notes:
  - Implementation assumes `read()`-capable device and RGB24 mode. This works for many setups; some exotic devices may need additional handling.
...
```go
// Frame is a single RGB24 frame from the camera.
type Frame struct {
    Data   []byte // len(Data) == Width * Height * 3
    Width  int
    Height int
}
